...
describe("middleware", () => {
  it("wraps a handler with cross-cutting behavior", async () => {
    const events: string[] = [];
    const handler: Handler = async () => ({ status: 200, body: { ok: true } });
    const wrapped = chainMiddleware(auditPath(events), requireUser())(handler);
  ...
  it("can stop the chain early", async () => {
    ...
    await expect(wrapped({ user: "", path: "/documents" })).resolves.toEqual({
      status: 401,
