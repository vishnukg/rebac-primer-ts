...
export const makeGraphEvaluator = ({
  ...
  const hasRelation = async (
    ...
    try {
      ...
      if (objectType === objectTypes.team) {
        return expandByRules(resolution, teamRules, user, object, rel, depth);
      }
  ...
  const hasTuple = async (
    ...
    if (direct) {
      resolution.trace.push(`Found direct tuple (${object}, ${rel}, ${user})`);
      return true;
    }
  ...
  const expandByRules = async (
    ...
    for (const implied of rules[rel] ?? []) {
      ...
      if (await hasRelation(resolution, user, object, implied, depth + 1)) {
        return true;
      }
    }
  ...
  const expandDocument = async (
    ...
    if (
      await expandByRules(resolution, documentRules, user, object, rel, depth)
    ) {
      return true;
    }
    ...
    for (const parent of parents) {
      ...
      try {
        if (parseObject(workspaceObject).type !== objectTypes.workspace) {
          continue;
        }
      } catch {
        continue;
      ...
      if (
        await hasRelation(resolution, user, workspaceObject, rel, depth + 1)
      ) {
        return true;
      }
    }
