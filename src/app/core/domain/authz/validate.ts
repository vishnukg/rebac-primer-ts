...
const relationCheckableForUser = (
  ...
  !(objectType === objectTypes.document && rel === relation.documentWorkspace);
...
const subjectAllowed = (
  ...
  if (objectType === objectTypes.workspace && rel === relation.workspaceOwner) {
...
export const validateTuple = (tuple: TupleKey): void => {
  ...
  if (
    !subjectAllowed(objectType, tuple.relation, subject.type, subject.relation)
  ) {
    throw tupleValidationError(
